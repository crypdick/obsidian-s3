		...
		new Setting(contentEl).addButton((b) => {
			...
			b.onClick(async () => {
				...
				try {
					...
					console.log("[s3-attachments] convertAttachments report:", report);
					if (report.previewLines?.length) {
						console.log("[s3-attachments] convertAttachments preview (first 200):\n" + report.previewLines.slice(0, 200).join("\n"));
					}
					if (report.errors?.length) {
						console.warn("[s3-attachments] convertAttachments errors:\n" + report.errors.join("\n"));
					}
					this.close();
